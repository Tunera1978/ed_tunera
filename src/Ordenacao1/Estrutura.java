...
public class Estrutura {
    private dinossauros[] a;                 // cria a referência para um vetor do tipo dinossauros
   private int nElems;               // nElems controla quantos elementos o vetor terá ...
   //-----------------------------------------------------------
   public Estrutura(int max)         // método construtor
      {
      a = new dinossauros[max];                 // cria o vetor ...
      ...
      while(true)
         ...
         if(a[curIn].getNome().charAt(0) == searchKey.charAt(0)) {  //mudado p/ desafio ...
             ...
             for (int i = curIn; (a[i].getNome().charAt(0) == searchKey.charAt(0)); i++) { //mudado p/desafio
                 a[i].displaydinossauros(); //mudado p/ imprimir todos os dados da pessoa
                 System.out.println("==============================================");
             }
             for (int j = curIn-1; (a[j].getNome().charAt(0) == searchKey.charAt(0)); j--) { //mudado p/ desafio
                   a[j].displaydinossauros();//mudado p/ imprimir todos os dados da pessoa
                 System.out.println("==============================================");
             }   
             return true;
         }
...
//--------------------------------------------------------------
   public void insert(String n, String p, int id)    // insere o elemento no vetor
      ...
      for(int k=nElems; k>j; k--)    // move os elementos maiores uma posição p/ frente
         a[k] = a[k-1];
      //Apos abrir um espaço no lugar certo, ele cria o nome objeto dinossauros na posição aberta!!!
      a[j] = new dinossauros(n, p, id);                  // insere o elemento na posição necessária
...
//--------------------------------------------------------------
   ...
   //-----------------------------------------------------------
      ...
      for(out=1; out<nElems; out++)     // vamos rodar até o final do vetor
         {
         dinossauros temp = a[out];            // jogamos em uma variável (dinossauros) temporária o item marcado
